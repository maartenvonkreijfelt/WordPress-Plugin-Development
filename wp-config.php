...
define('DB_NAME', 'wordpress2local');
 ...
 * inside the /wp-content/ directory. This is useful if you want to review all notices later or need
 * to view notices generated off-screen (e.g. during an AJAX request or wp-cron run).
 *
 * Note that this allows you to write to /wp-content/debug.log using PHP's built in error_log() function,
 ...
 * is 'true' which shows errors and warnings as they are generated.
 * the debug info is saved in /wp-content/debug.log
 * Setting this to false will hide all errors. This should be used in conjunction
 * with WP_DEBUG_LOG so that errors can be reviewed later.
 */
define('WP_DEBUG_DISPLAY', false);

/*
 * The SAVEQUERIES definition saves the database queries to an array and
 * that array can be displayed to help analyze those queries. The constant defined as true
 * causes each query to be saved, how long that query took to execute, and what function called it.
 */
